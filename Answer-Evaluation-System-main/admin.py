...
def view_student_scores(student_id):
    if 'admin_logged_in' in session:
        ...
        # Convert result tuples into list of dictionaries
        scores = [
            {
                'answer_id': score[0],
                'test_id': score[1],
...
def student_view_score():
    ...
    # 2) Fetch everything, including question_id
    query = """
        SELECT
            t.test_id,
            t.test_name,
    ...
    for test_id, test_name, question_id, question_text, expected_answer, student_answer in results:
        ...
        # 5) Update the score in the DB using the known question_id
        cur.execute("""
        ...
        student_scores[test_id]['max_score']   += 10  # or whatever your per‑question max is
